...
The following is a set of guidelines for contributing to any [craquehouse](https://github.com/craquehouse) repository. Individual repos may add or refine these rules in their own `CONTRIBUTING.md`; in that case, the repo-level guide takes precedence over this one.
...
> [!IMPORTANT]
> AI tools are welcome in [craquehouse](https://github.com/craquehouse) repositories, but their use **must be clearly disclosed**. If any portion of a contribution — code, docs, bug reports, pull request descriptions, or discussion posts — was generated or substantially shaped by AI, say so plainly.

If AI is used to generate any portion of a change, contributors must:

1. Explicitly disclose that AI was used and describe how (e.g., which tool, and for what parts of the change).
2. Perform a comprehensive manual review before submitting the pull request.
3. Be prepared to explain every line submitted when asked about it by a reviewer.

You are always accountable for what you submit, regardless of how it was produced. Undisclosed AI use, or contributions you cannot stand behind and explain, may result in your changes being rejected.

Individual repositories may publish stricter or more detailed AI policies (e.g., in an `AGENTS.md` file); when they do, the repo-level policy takes precedence.
